...
>! Due espressioni possono sembrare diverse e valere lo stesso: prima di dichiarare falsa un'identità, conviene controllarla con un valore numerico dell'angolo (per esempio $x=30°$) invece di fidarsi solo dell'aspetto delle due scritture.` }
  ...
  grafici: {
    ...
    angoloAggiunto: {
      ...
      parametri: [
        ...
        { nome: 'b', min: -3, max: 3, passo: 0.1, valore: 4, etichetta: 'b' }
      ],
      funzioni: [
  ...
  formulario: [
    { nome: 'Addizione del seno', formula: R`\sin(\alpha+\beta) = \sin\alpha\cos\beta + \cos\alpha\sin\beta`, nota: R`Per la sottrazione il segno centrale non cambia: $\sin(\alpha-\beta)=\sin\alpha\cos\beta-\cos\alpha\sin\beta$.` },
  ...
  quiz: [
    ...
    { id: 'q-16', domanda: R`La formula $\cos^2x=\dfrac{1+\cos2x}{2}$ si ottiene…`, opzioni: [R`Dal teorema di Tolomeo`, R`Dalla formula dell'angolo aggiunto`, R`Dalla formula di prostaferesi per due coseni`, R`Da Werner con $\cos\alpha\cos\beta$, ponendo $\alpha=\beta=x$`], corretta: 3, spiegazione: R`Werner dà $\cos\alpha\cos\beta=\frac12[\cos(\alpha-\beta)+\cos(\alpha+\beta)]$; con $\alpha=\beta=x$ diventa $\cos^2x=\frac12[1+\cos2x]$.` }
  ],
