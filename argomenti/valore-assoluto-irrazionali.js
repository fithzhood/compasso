...
>! Dimenticare il primo dei due sistemi (quello con $B(x) < 0$) è l'errore più comune nelle disequazioni con «$>$»: si perdono tutte le soluzioni in cui il secondo membro è negativo, che sono valide senza bisogno di calcolare nulla.` }
  ...
  quiz: [
    ...
    { id: 'q-08', domanda: R`Se $k \le 0$, la disequazione $|A(x)| > k$…`, opzioni: [R`non ha soluzione`, R`equivale a $A(x)>k$`, R`equivale a $A(x)<-k$`, R`è vera per ogni $x$ del dominio di $A$`], corretta: 3, spiegazione: R`$|A(x)|\ge 0$ sempre, e $0 \ge k$ quando $k\le 0$: il modulo supera automaticamente un numero non positivo, in ogni punto in cui $A(x)$ è definita.` },
  ...
  suggerimenti: [
    ...
    { tipo: 'metodo', testo: R`Con $|A|<k$ e $|A|>k$, se $k$ non è positivo non serve fare calcoli: ragiona subito sul fatto che $|A|\ge 0$.` },
