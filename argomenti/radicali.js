...
Il grafico mostra $y = x^{\frac{1}{n}}$ per $x \ge 0$: al crescere di $n$ (radici di indice sempre più alto) la curva si appiattisce vicino a $x=0$ e cresce più lentamente, perché estrarre una radice di indice alto "smorza" molto di più i numeri grandi.
...
>* **Identità:** per $p, q \ge 0$, $$\sqrt{(p+q) \pm 2\sqrt{pq}} = \sqrt{p} \pm \sqrt{q}$$ (si verifica elevando al quadrato il membro destro).
...
>! Elevare al quadrato non è un'operazione reversibile: può introdurre soluzioni che non risolvono l'equazione di partenza. Ogni soluzione trovata va sempre verificata (o confrontata con le condizioni scritte all'inizio), altrimenti si rischia di accettare un valore sbagliato.` }
  ...
  grafici: {
    ...
    esponenteN: {
      ...
      didascalia: "Trascina il punto sull'asse x e cambia n con il cursore: piu' n cresce, piu' la curva y = x^(1/n) si appiattisce vicino a x = 0."
    }
  },
  ...
  formulario: [
    ...
    { nome: 'Radicale doppio', formula: R`\sqrt{(p+q) \pm 2\sqrt{pq}} = \sqrt{p} \pm \sqrt{q}`, nota: R`Con $p, q \ge 0$; utile quando $p+q=A$ e $pq=B$ sono numeri semplici.` }
  ],
